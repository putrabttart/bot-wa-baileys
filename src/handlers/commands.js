    ...
    try {
      ...
      if (/^#menu$/i.test(text)) {
        await msg.reply([
          ...
          '• #ping',
          '• #kategori',
          `• #list [kategori] [hal]  (nama produk saja, ${ENV.LIST_PER_PAGE}/hal)`,
          '• #list all               (tampilkan semua produk)',
          '• #stok [kategori] [hal]  (tampilkan detail lengkap)',
          '• #harga <keyword>',
          '• #detail <kode>',
          '• #beli <kode>',
      ...
      if (/^#list\s+all\s*$/i.test(text)) {
        ...
        const names = data.map((it, i) => `${i + 1}. ${it.nama || '-'}`);
        const batches = chunk(names, 200);
        for (let i = 0; i < batches.length; i++) {
          const header = i === 0 ? `🧾 Semua Produk (${data.length})` : `Bagian ${i + 1}`;
          await msg.reply([header, ...batches[i]].join('\n'));
      ...
      if (/^#list\b/i.test(text)) {
        ...
        const per = Math.max(1, Number(ENV.LIST_PER_PAGE || 20));
        const { items, page: p, total } = paginate(data, page, per);
        if (!items.length) {
          await msg.reply(cat ? `Tidak ada produk untuk kategori *${cat}*.` : 'Belum ada produk.');
          try { await msg.react('❌'); } catch {}
          return;
        }
        const startIndex = (p - 1) * per;
        const names = items.map((it, i) => `${startIndex + i + 1}. ${it.nama || '-'}`);
        const header = '🧾 Daftar Produk';
        const footer = `\nHalaman ${p}/${total} — *#list ${cat ? cat + ' ' : ''}${p + 1}* untuk berikutnya.`;
        const body = [header, ...names, footer].join('\n');
        await msg.reply(body);
        try { await msg.react('✅'); } catch {}
        return;
      }

      /* ---------- #stok (detail lengkap, seperti list sebelumnya) ---------- */
      if (/^#stok\b/i.test(text)) {
        await loadProducts();
        const parts = text.split(/\s+/).slice(1);
        let cat = ''; let page = 1;
        if (parts.length === 1 && /^\d+$/.test(parts[0])) page = Number(parts[0]);
        ...
        let data = getProducts();
        if (cat) data = data.filter(p => (p.kategori || '').toLowerCase().includes((cat || '').toLowerCase()));
        const per = Math.max(1, Number(ENV.STOK_PER_PAGE || 8));
        const { items, page: p, total } = paginate(data, page, per);
        if (!items.length) {
          await msg.reply(cat ? `Tidak ada produk untuk kategori *${cat}*.` : 'Belum ada produk.');
          try { await msg.react('❌'); } catch {}
        ...
        await msg.reply(chunks.join('\n\n') + `\n\nHalaman ${p}/${total} — *#stok ${cat ? cat + ' ' : ''}${p + 1}* untuk berikutnya.`);
        try { await msg.react('✅'); } catch {}
        return;
      }
